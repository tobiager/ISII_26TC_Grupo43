...
CREATE TABLE alergia (
  ...
  nombre_alergia VARCHAR(200) NOT NULL
);
...
CREATE TABLE ficha_alergia (
  ...
  CONSTRAINT pk_ficha_alergia PRIMARY KEY (id_ficha_medica, id_alergia),
  CONSTRAINT fk_ficha_alergia_ficha_medica FOREIGN KEY (id_ficha_medica) REFERENCES ficha_medica(id_ficha_medica),
  CONSTRAINT fk_ficha_alergia_alergia FOREIGN KEY (id_alergia) REFERENCES alergia(id_alergia)
);

CREATE TABLE ficha_enfermedad_cronica (
  id_ficha_medica INT NOT NULL,
  id_enfermedad_cronica INT NOT NULL,
  CONSTRAINT pk_ficha_enfermedad_cronica PRIMARY KEY (id_ficha_medica, id_enfermedad_cronica),
  CONSTRAINT fk_ficha_enf_cronica_ficha_medica FOREIGN KEY (id_ficha_medica) REFERENCES ficha_medica(id_ficha_medica),
  CONSTRAINT fk_ficha_enf_cronica_enfermedad FOREIGN KEY (id_enfermedad_cronica) REFERENCES enfermedad_cronica(id_enfermedad_cronica)
);

CREATE TABLE ficha_antecedente_familiar (
  id_ficha_medica INT NOT NULL,
  id_antecedente_familiar INT NOT NULL,
  CONSTRAINT pk_ficha_antecedente_familiar PRIMARY KEY (id_ficha_medica, id_antecedente_familiar),
  CONSTRAINT fk_ficha_ant_familiar_ficha_medica FOREIGN KEY (id_ficha_medica) REFERENCES ficha_medica(id_ficha_medica),
  CONSTRAINT fk_ficha_ant_familiar_antecedente FOREIGN KEY (id_antecedente_familiar) REFERENCES antecedente_familiar(id_antecedente_familiar)
);
...
CREATE TABLE paciente (
  ...
  CONSTRAINT uq_id_persona UNIQUE (id_persona)
);
...
CREATE TABLE afiliacion_obra_social (
  ...
  CONSTRAINT fk_afiliacion_paciente FOREIGN KEY (id_paciente) REFERENCES paciente(id_paciente),
  CONSTRAINT fk_afiliacion_obra_social FOREIGN KEY (id_obra_social) REFERENCES obra_social(id_obra_social),
  CONSTRAINT uq_afiliacion_numero UNIQUE (numero_afiliado),
  CONSTRAINT ck_afiliacion_fechas CHECK (fecha_vencimiento >= fecha_alta),
  CONSTRAINT ck_afiliacion_fecha_baja CHECK (fecha_baja IS NULL OR fecha_baja >= fecha_alta)
);
...
CREATE INDEX idx_afiliacion_paciente ON afiliacion_obra_social(id_paciente);

CREATE INDEX idx_afiliacion_obra_social ON afiliacion_obra_social(id_obra_social);

CREATE INDEX idx_ficha_alergia_ficha_medica ON ficha_alergia(id_ficha_medica);

CREATE INDEX idx_ficha_enf_cronica_ficha_medica ON ficha_enfermedad_cronica(id_ficha_medica);

CREATE INDEX idx_ficha_ant_familiar_ficha_medica ON ficha_antecedente_familiar(id_ficha_medica);
...
ALTER TABLE afiliacion_obra_social ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "solo_autenticados" ON afiliacion_obra_social FOR ALL TO authenticated USING (true);
